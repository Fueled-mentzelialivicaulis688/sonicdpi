...
//! next to the CLI exe so it works when the user double-clicks it.
...
fn copy_windivert_files() -> std::io::Result<()> {
    ...
    // Repo root: target/{profile}/.. = workspace root
    // OUT_DIR layout: target/{profile}/build/{crate}-{hash}/out
    //   profile_dir == target/{profile}
    //   profile_dir.parent() == target/
    //   profile_dir.parent().parent() == workspace root
