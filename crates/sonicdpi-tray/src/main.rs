...
impl TrayApp {
    ...
    fn build_menu(&mut self) -> Menu {
        ...
        let _ = menu.append(&PredefinedMenuItem::separator());
        // VPN-сервисы автора — на случай если DPI-обхода не хватает
        let vpn_sub = Submenu::new("VPN от автора (если SonicDPI не справляется)", true);
        let bot_galevpn = MenuItem::new("Telegram: @galevpn_bot", true, None);
        self.menu_ids.bot_galevpn = bot_galevpn.id().clone();
        let _ = vpn_sub.append(&bot_galevpn);
        let bot_bysonicvpn = MenuItem::new("Telegram: @bysonicvpn_bot", true, None);
